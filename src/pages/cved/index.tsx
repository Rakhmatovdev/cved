    ...
    <div className="cved-page">
      ...
      {mode === "ai" ? (
        <section className="cved-ai-shell">
          <aside className="cved-ai-context"><div className="cved-ai-context-head"><div className="cved-ai-logo"><Bot size={20} /></div><div><strong>{t("cved.ai_name")}</strong><span>{t("cved.ai_status")}</span></div></div><button type="button" className="cved-new-session" onClick={() => { setMessages([{ id: Date.now(), role: "assistant", text: t("cved.greeting") }]); setDraft(""); setAttachedFile(null); setFileError(""); setIsTyping(false); }}><Sparkles size={15} /> {t("cved.new_session")}</button><p className="cved-context-label">{t("cved.capabilities")}</p><div className="cved-capability"><CheckCircle2 size={16} /><span>{t("cved.capability_review")}</span></div><div className="cved-capability"><CheckCircle2 size={16} /><span>{t("cved.capability_security")}</span></div><div className="cved-capability"><CheckCircle2 size={16} /><span>{t("cved.capability_explain")}</span></div><div className="cved-context-metrics"><div><strong>128k</strong><span>{t("cved.context_window")}</span></div><div><strong>24/7</strong><span>{t("cved.availability")}</span></div></div><div className="cved-ai-context-note"><ShieldCheck size={15} /><span>{t("cved.privacy_note")}</span></div></aside>
          <div className="cved-ai-chat"><header className="cved-ai-chat-head"><div className="cved-ai-agent"><div className="cved-ai-avatar"><Bot size={18} /></div><div><h2>{t("cved.ai_name")}</h2><span><i /> {t("cved.ai_status")}</span></div></div><span className="cved-ai-private"><Check size={13} /> {t("cved.private")}</span></header><div className="cved-ai-messages"><div className="cved-ai-welcome"><div className="empty-orbit"><Sparkles size={23} /></div><h3>{t("cved.welcome_title")}</h3><p>{t("cved.welcome_text")}</p></div>{messages.map((message) => <div className={`cved-ai-message ${message.role}`} key={message.id}><div className="cved-ai-message-avatar">{message.role === "assistant" ? <Bot size={14} /> : <User size={14} />}</div><div className="cved-ai-bubble"><p>{message.text}</p>{message.file && <div className="cved-ai-file"><FileText size={14} />{message.file}</div>}</div></div>)}{isTyping && <div className="cved-ai-message assistant"><div className="cved-ai-message-avatar"><Bot size={14} /></div><div className="cved-ai-bubble cved-ai-typing"><i /><i /><i /></div></div>}<div ref={messagesEndRef} /></div><div className="cved-ai-composer-wrap"><div className="cved-ai-prompts">{prompts.map((prompt) => <button type="button" key={prompt} onClick={() => sendMessage(prompt)}>{prompt}</button>)}</div>{attachedFile && <div className="cved-ai-file-preview"><FileText size={14} />{attachedFile}<button type="button" onClick={() => { setAttachedFile(null); setFileError(""); }} aria-label={t("cved.remove_file")}><X size={14} /></button></div>}{fileError && <p className="cved-ai-file-error">{fileError}</p>}<div className="cved-ai-composer"><button type="button" className="chat-icon-button" onClick={() => fileInputRef.current?.click()} aria-label={t("cved.attach_file")}><Paperclip size={18} /></button><input ref={fileInputRef} type="file" className="sr-only" accept=".js,.jsx,.ts,.tsx,.py,.java,.cpp,.c,.go,.txt,.pdf" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; if (file.size > 25 * 1024 * 1024) { setFileError(t("cved.file_too_large")); setAttachedFile(null); return; } setFileError(""); setAttachedFile(file.name); event.target.value = ""; }} /><textarea maxLength={4000} value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendMessage(); } }} placeholder={t("cved.placeholder")} rows={1} /><button type="button" className="chat-send" onClick={() => sendMessage()} disabled={!draft.trim() && !attachedFile} aria-label={t("cved.send")}><Send size={17} /></button></div><p className="cved-ai-note">{t("cved.keyboard_hint")}</p></div></div>
        </section>
      ) : (
